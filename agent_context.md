...
5. **HDF5 Event Numbering & Zero-Padding**: All event groups under `runs/{run_idx}/events/` in HDF5 files must strictly use 3-digit zero-padded string keys (`000` for the placeholder/no-data event, and `001`, `002`, ... `999` for valid events). In contrast, `runs` groups use unpadded integers (`0`, `1`, ... `6`). This zero-padding guarantees that ASCII/lexicographical sorting in external HDF5 tools matches chronological numerical order. In `DataManager`, `save_file()` and `fast_save_event_analysis()` explicitly enforce `f"{idx:03d}"` for events. When reading via `_load_hdf5()`, `int(k) == i` is used, which seamlessly converts both zero-padded (`001`) and legacy unpadded (`1`) keys into 0-based Python lists.
