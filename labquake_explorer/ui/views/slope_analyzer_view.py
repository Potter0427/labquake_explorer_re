...
class SlopeAnalyzerView(tk.Toplevel):
    def __init__(self, parent, item_y=None, item_x=None):
        ...
        self.base_path = None
        if item_y:
            self.base_path = posixpath.dirname(item_y)
            self.item_y = posixpath.basename(item_y)
        else:
            self.item_y = None
        self.item_x = item_x
    ...
    def plot_data(self):
        ...
        if self.item_x is None:
            self.ax.clear()
            full_path = posixpath.join(self.base_path, self.item_y) if self.base_path else self.item_y
            ...
            self.ax.clear()
            x_path = posixpath.join(self.base_path, self.item_x) if self.base_path else self.item_x
            y_path = posixpath.join(self.base_path, self.item_y) if self.base_path else self.item_y
