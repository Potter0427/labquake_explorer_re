...
class SignalUtils:
    ...
    # @staticmethod
    # def calculate_local_shear(tau_raw, area=0.015):
    #     """
    #     計算局部剪應力 (Local Shear Stress)
    #     基於實驗校正係數：
    #     voltage = raw_tau / 3.065
    #     shear = voltage * 6.482 * 0.01267 / area
    #     """
    #     # 避免分母為 0
    #     if area <= 0:
    #         raise ValueError("Sliding area must be greater than 0.")
            
    #     voltage_ch9 = tau_raw / 3.065
    #     local_shear = voltage_ch9 * 6.482 * 0.01267 / area
    #     return local_shear
...
class LabQuakeAnalyzer:
    ...
    def save_processed_data(self, output_name, baseline_block=2, sliding_area=None, threshold=3, dt=0.2, lowpass=500):
        # 1. 先抓出這份檔案到底有多少個 Block (偵測所有觸發事件)
        from tpc5_data_processor import TPC5DataProcessor
        with TPC5DataProcessor(self.filename) as proc:all_blocks = proc.get_available_blocks(self.params['sigma_ch'])
        
        print(f"Detecting {len(all_blocks)} blocks from {self.filename}...")
        data_dict = {}
        ...
        for block in all_blocks:
            ...
            # --- A. 如果是 Block 1 (連續資料)：照你原本的寫法，存所有東西 ---
            if block == 1:
                data_dict['time'] = t_b
                ...
                # LVDT
                _, lvdt = self.get_physical_data(self.params['lvdt_ch'], block, baseline_block=baseline_block)
                data_dict['lvdt'] = lvdt

                # Local Tau
                if sliding_area:
                    tau_local = self.calculate_local_shear(block, sliding_area)
                    data_dict['tau_local'] = tau_local

                # 預算 Block 1 全域背景速率
                for ch in self.params['eddy_chs']:
                    _, slip = self.get_physical_data(ch, block, baseline_block=baseline_block)
                    data_dict[f'slip_ch{ch}'] = slip 
                    
                    # A. Raw Rate + Filter
                    u_r, t_r = SignalUtils.threshold_sampling(slip, t_b, threshold=threshold, dt_max=dt)
                    if len(u_r) > 1:
                        rate = np.abs(np.gradient(u_r, t_r))
                        mask_v = rate > 1e-2 # 這裡過濾
                        data_dict[f'sliprate_ch{ch}'] = rate[mask_v]
                        data_dict[f't_sliprate_ch{ch}'] = np.array(t_r)[mask_v]
                    
                    # B. Lowpass Rate + Filter
                    s_lp, _ = SignalUtils.lowpass(slip, t_b, fc=500)
                    u_l, t_l = SignalUtils.threshold_sampling(s_lp, t_b, threshold=threshold, dt_max=dt)
                    if len(u_l) > 1:
                        rate = np.abs(np.gradient(u_l, t_l))
                        mask_v = rate > 1e-2 # 這裡過濾
                        data_dict[f'lp_ch{ch}'] = rate[mask_v]
                        data_dict[f't_lp_ch{ch}'] = np.array(t_l)[mask_v]

            # --- 針對高取樣 Block ---
            else:
                for ch in self.params['eddy_chs']:
                    _, slip = self.get_physical_data(ch, block, baseline_block=baseline_block)
                    s_lp, _ = SignalUtils.lowpass(slip, t_b, fc=lowpass)
                    u_dec, t_dec = SignalUtils.threshold_sampling(s_lp, t_b, threshold=threshold, dt_max=dt)
                    
                    if len(u_dec) > 1:
                        rate_b = np.abs(np.gradient(u_dec, t_dec))
                        mask_v = rate_b > 1e-2 # 這裡過濾
                        data_dict[f'high_sliprate_ch{ch}_blk{block}'] = rate_b[mask_v]
                        data_dict[f't_high_sliprate_ch{ch}_blk{block}'] = np.array(t_dec)[mask_v]

        # 3. 存下所有的 Trigger Time 資訊
        trigs = self.get_trigger_times(self.params['sigma_ch'])
        data_dict['trigger_times'] = trigs
